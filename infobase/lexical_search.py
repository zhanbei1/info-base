...
from infobase.configs import logger
...
class CustomIndex:
    ...
    def index_document(self, doc_id, content):
        ...
        for token in self.tokenizer(content):
            # 排除单词中的一些常见词语
            if token in BaseConfig.exclude_words["all_set"]:
                continue
            else:
                # nltk 自然语言预处理
...
def search_docs(query, index: CustomIndex):
    ...
    # TODO：拆分query中的每个token，找到代码解析之后每个代码chunks，
    #  然后拆分token。用query的token找到每个chunks中包含这个token的docu_id
    #  然后根据doc_id， token 和token出现的频率，进行 BM25 评分
    #  然后再做归一化处理
    if len(res) == 0:
        max_score = 1
        min_score = 0
